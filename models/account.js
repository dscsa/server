...
exports.inventory = function* (id) { //account._id will not be set because google does not send cookie
  ...
  this.body = csv.fromJSON(rows, ["_id","qty.bins","qty.repack","qty.pending","ordered","order.maxInventory", "order.minQty", "order.minDays","order.verifiedMessage","order.destroyedMessage", "order.defaultBin","order.price30","order.price90","order.vialQty","order.vialSize"])
}
